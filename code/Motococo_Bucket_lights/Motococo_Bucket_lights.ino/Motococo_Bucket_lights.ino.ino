...
#define NUMPIXELS      3     // MOTOCOCO! Number of Neopixels vary
...
//  sensorValue = analogRead(POTI_LEFT);
//  mappedValue = map(sensorValue, 0, 1023, 0, 8);
//  
//  uint8_t pixNum = random(20);
//  uint8_t colorR = random(255);
//  uint8_t colorG = random(255);   
//  uint8_t colorB = random(255);
//  for(uint16_t i=0; i<mappedValue; i++) {  
...
// pixels.setPixelColor(random(NUMPIXELS), pixels.Color(random(0), random(0), random(0)));
// pixels.show();
// delay(250);
...
theaterChase(pixels.Color(255, 255, 255), random(1500));
//colorWipe(pixels.Color(0, 255, 0), 50);

delay(random(19000));

// When Flashlight hits sensor -> chase effect 
